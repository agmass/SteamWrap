...
struct Event
{
	...
	int m_success;
	value m_data;
	Event(const char *type, bool success = false, const std::string &data = "") : m_type(type), m_success(success), m_data(alloc_string(data.c_str())) {}
	Event(const char *type, bool success, value data) : m_type(type), m_success(success), m_data(data) {}
};
...
public:
	CallbackHandler() : m_CallbackUserStatsReceived(this, &CallbackHandler::OnUserStatsReceived),
...
#pragma region UGC
	...
	//-----------------------------------------------------------------------------------------------------------
	value SteamWrap_OpenOverlayToURL(value url)
	{
	...
	DEFINE_PRIM(SteamWrap_OpenOverlayToURL, 1);

	//-----------------------------------------------------------------------------------------------------------
	value SteamWrap_OpenOverlay(value dialog)
	{
		if (!val_is_string(dialog) || !CheckInit())
		{
			return alloc_bool(false);
		}

		SteamFriends()->ActivateGameOverlay(val_string(dialog));
		return alloc_bool(true);
	}
	...
	value SteamWrap_AddUGCItemKeyValueTag(value updateHandle, value keyStr, value valueStr)
	{
		if (!CheckInit())
			return alloc_bool(false);
		if (!val_is_string(updateHandle))
			return alloc_bool(false);
		if (!val_is_string(keyStr))
			return alloc_bool(false);
		if (!val_is_string(valueStr))
			return alloc_bool(false);
...
#pragma region Scores /Achievements
	...
	{
		if (!val_is_string(name) || !val_is_string(key) || !CheckInit())
			return alloc_string("");
...
#pragma region New Workshop
	...
	value SteamWrap_GetItemDownloadInfo(value publishedFileID)
	{
		if (!CheckInit())
			return alloc_string("");
		if (!val_is_string(publishedFileID))
			return alloc_string("");
	...
	value SteamWrap_GetItemInstallInfo(value publishedFileID, value maxFolderPathLength)
	{
		...
		if (!val_is_string(publishedFileID))
			return alloc_string("");
		if (!val_is_int(maxFolderPathLength))
			return alloc_string("");
	...
	*/
	...
	value SteamWrap_GetQueryUGCKeyValueTag(value cHandle, value iIndex, value iKeyValueTagIndex, value keySize, value valueSize)
	{
		if (!CheckInit())
			return alloc_string("");
		if (!val_is_string(cHandle))
			return alloc_string("");
		if (!val_is_int(iIndex))
			return alloc_string("");
		if (!val_is_int(iKeyValueTagIndex))
			return alloc_string("");
		if (!val_is_int(keySize))
			return alloc_string("");
		if (!val_is_int(valueSize))
			return alloc_string("");
	...
	value SteamWrap_GetQueryUGCMetadata(value sHandle, value iIndex, value iMetaDataSize)
	{
		if (!CheckInit())
			return alloc_string("");
		if (!val_is_string(sHandle))
			return alloc_string("");
		if (!val_is_int(iIndex))
			return alloc_string("");
		if (!val_is_int(iMetaDataSize))
			return alloc_string("");
...
#pragma region Old Workshop
	...
	value SteamWrap_GetUGCDownloadProgress(value contentHandle)
	{
		if (!CheckInit())
			return alloc_string("");
		if (!val_is_string(contentHandle))
			return alloc_string("");
	...
	value SteamWrap_UGCRead(value handle, value bytesToRead, value offset, value readAction)
	{
		...
		if (u64Handle == 0 || cubDataToRead == 0)
			return alloc_string("");
...
#define SteamNetworking SteamNetworking()
	...
	value SteamWrap_SendPacket(value handle, value haxeBytes, value size, value type)
	{
		...
		{
		case 1:
			etype = k_EP2PSendUnreliableNoDelay;
			break;
		case 2:
			etype = k_EP2PSendReliable;
			break;
		case 3:
			etype = k_EP2PSendReliableWithBuffering;
			break;
		}
		if (bytes.data == 0)
			return alloc_bool(false);
	...
	{
		if (!CheckInit() || SteamWrap_PacketData == nullptr)
			return alloc_bool(false);
...
#pragma region Current lobby
	...
	value SteamWrap_LobbyGetData(value lobby, value field)
	{
		if (CheckInit() && val_is_string(field) && val_is_string(lobby) && hx_to_id(lobby).IsValid())
		{
			return alloc_string(SteamMatchmaking()->GetLobbyData(hx_to_id(lobby), val_string(field)));
		}
		else
			return alloc_string("0");
	}
...
#pragma region Lobby list filters
	...
	value SteamWrap_LobbyListAddDistanceFilter(value mode)
	{
		...
		{
		case 0:
			d = k_ELobbyDistanceFilterClose;
			break;
		case 1:
			d = k_ELobbyDistanceFilterDefault;
			break;
		case 2:
			d = k_ELobbyDistanceFilterFar;
			break;
		case 3:
			d = k_ELobbyDistanceFilterWorldwide;
			break;
		}
...
#pragma region Steam Controller
	...
	void SteamWrap_TriggerHapticPulse(int controllerHandle, int targetPad, int durationMicroSec)
	{
		...
		{
		case 0:
			eTargetPad = k_ESteamControllerPad_Left;
		case 1:
			eTargetPad = k_ESteamControllerPad_Right;
		default:
			eTargetPad = k_ESteamControllerPad_Left;
		}
